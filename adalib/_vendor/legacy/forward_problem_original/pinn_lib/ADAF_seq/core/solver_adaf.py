...
from .model_adaf import ADAF_Reusable
...
class Solver:
    ...
    def train_adam_lbfgs_piecewise(
        ...
        THIS is the actual "sequential" loop referenced from
        adalib/forward/solver.py's docstring: the `for k in
        range(self.n_seg)` loop below processes segments strictly in
        order, k=0,1,...,n_seg-1. Segment k+1 can only start once
        segment k is fully trained, because k+1's own initial condition
        (`init1`/`init2` below) is read off from k's converged endpoint
        — there is no way to train two segments in parallel or out of
        order. Each segment is otherwise a self-contained ADA-F fit:
        Adam minimizes the physics residual over that segment's
        Nt_seg collocation points, optionally polished by L-BFGS.
        """
        ...
        # SEQUENTIAL loop: k = 0, 1, ..., n_seg-1, strictly in order. Every
        # iteration depends on the previous one's result (see `init1`/
        # `init2` below, and the continuity update at the bottom of the
        # loop body) — segment k+1 literally cannot be trained before
        # segment k finishes, unlike e.g. training on independent batches.
        for k in range(self.n_seg):
            ...
            # set inits for this segment — on the first iteration (k=0)
            # these are the user's true x0; on every later iteration they
            # are `next_init1`/`next_init2` computed from segment k-1's
            # converged endpoint (see bottom of this loop). This is what
            # stitches the segments into one continuous trajectory.
            for j in range(self.ode_num):
                self.models[j].set_inits(init1[j], init2[j], zero3)

            # warm-start: reuse the PREVIOUS segment's converged basis
            # weights W as this segment's starting point (instead of
            # random init). Consecutive segments usually have similar
            # dynamics, so this makes each new segment converge faster.
            for j in range(self.ode_num):
            ...
            # This is the hand-off to the NEXT loop iteration: read the
            # just-trained segment's value/slope at its LAST collocation
            # point (y_seg_tf[-1], dy_dx_tf[-1]) and stash them as
            # next_init2/next_init1. A few lines below, `init1 =
            # next_init1; init2 = next_init2` makes these become segment
            # (k+1)'s `init1[j]`/`init2[j]` at the top of the loop above.
            next_init1 = []
            next_init2 = []
